...
9. Update execution status, authorization, navigation, and the handoff map so roadmap phase, lifecycle, milestone, continuation, readiness, owner, and required action agree. Codex-owned authorized work uses `CONTINUE`; Antigravity uses `WAIT_FOR_AGENT`; user action uses `WAIT_FOR_USER`.
