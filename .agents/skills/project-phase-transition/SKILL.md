...
description: Close a project phase, audit every file in the active documentation tree, archive verified completed files, repair navigation, and prepare the next standalone active-gate README. Use when Codex is asked to mark a phase complete, perform end-of-phase cleanup, transition the active gate, clean completed handoffs or plans, archive stale active records, or prepare the next session.
---
...
1. Read `AGENTS.md`, `project_docs/INDEX.md`, `project_docs/active/README.md`, `project_docs/active/rules/DOCUMENTATION_GOVERNANCE.md`, `project_docs/active/status/project_execution_status.md`, `project_docs/active/status/phase_authorization.json`, `project_docs/active/active_gate/README.md`, the roadmap named by status, and the ownership guardrail.
...
8. Replace `project_docs/active/active_gate/README.md` with one detailed forward-looking gate and remove every other file or subdirectory from `active_gate/`. Choose the next work only from the accepted roadmap or explicit user direction. Never infer implementation authority: update `phase_authorization.json` only from direct user authorization, or create a `REVIEW_ONLY`/`NOT STARTED` gate when implementation is not authorized.
9. Update execution status, authorization, navigation, and the handoff map so phase identity, lifecycle, milestone, continuation, readiness, owner, and required action agree. Codex-owned authorized work uses `CONTINUE`; Antigravity uses `WAIT_FOR_AGENT`; user action uses `WAIT_FOR_USER`.
10. Re-run the full active-file inventory and completion search after edits. Then run `python .codex/hooks/agent_harness_check.py`, `python .codex/hooks/check_active_gate.py project_docs/active/active_gate .`, hook tests, and `git diff --check`. Treat every failure as a blocker.
11. Report what was archived, what intentionally remains active, the new gate, the next owner, the active-gate README path, and validation results. Do not paste a long prompt into chat unless the user explicitly requests it.
...
When the next owner is Codex, store the complete prompt only in `project_docs/active/active_gate/README.md`. When the next owner is Antigravity, store it under `project_docs/active/ai_hand_off/` and include exact frontend files, backend fields, acceptance checks, build command, and browser checklist.
