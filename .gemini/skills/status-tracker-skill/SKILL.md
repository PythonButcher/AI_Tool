...
The return command validates the actual frontend worktree and runs the full repository harness before changing ownership. It rejects missing or empty targets, suspicious shrinkage, any repository path outside phase authorization, frontend files outside handoff targets, missing required target changes, oversized diffs, forbidden inline styles, whitespace errors, and a return with no durable source diff. Never bypass a rejection with a bulk rewrite or Git restore command; stop and report it to Codex.
