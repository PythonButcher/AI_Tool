...
## Runtime Registration And Compatibility Inventory

The primary Flask application always registers `backend/routes/decision_chat.py`. This preserves `POST /api/decision/chat/turns` and the lazy compatibility adapter at `POST /api/decision/chat/actions` under the existing `/api/decision` prefix. `backend/routes/decision.py` is a compatibility-only blueprint. It is imported and registered only when Flask config `ENABLE_DECISION_INTELLIGENCE_COMPATIBILITY` is true or environment variable `ENABLE_DECISION_INTELLIGENCE_COMPATIBILITY` is one of `1`, `true`, `yes`, or `on`. The default is false.

| Classification | Routes or modules | Verified boundary |
| --- | --- | --- |
| Primary BI chat | `/api/decision/chat/turns`, `backend/routes/decision_chat.py`, BI paths in `backend/decision_engine/chat_service.py`, `grounding.py`, and `mode_detection.py` | Always registered. Normal Explore turns do not import workspace, output, graph, asset, pipeline, recommendation, or scenario services. |
| Lazy compatibility adapter | `/api/decision/chat/actions` and Decide-only methods inside `DecisionChatService` | URL remains stable. The adapter returns HTTP 409 with `DECISION_INTELLIGENCE_COMPATIBILITY_DISABLED` unless the compatibility flag is enabled. Workspace and output services are imported only after that explicit boundary permits execution. |
| Compatibility-only API | `/api/decision/workspaces`, `/api/decision/workspaces/analyze`, every `/api/decision/assets` route, every `/api/decision/graph` route, `/api/decision/signals/generate`, `/api/decision/brief/generate`, `/api/decision/run`, `/api/decision/recommendations/generate`, and `/api/decision/scenarios/evaluate` | Registered only through the explicit compatibility flag. Existing service and route tests remain the recovery contract. |
| Compatibility-only services | `decision_workspace_service.py`, `decision_output_service.py`, `decision_asset_service.py`, `decision_graph_service.py`, `decision_signal_service.py`, `decision_brief_service.py`, `decision_pipeline_service.py`, `recommendation_service.py`, and `scenario_service.py` | Not imported by primary startup or an ordinary BI turn. No service was deleted. |
| Shared support | `decision_support.py`, dataset resolution, governance readiness, metric resolution, relationship execution, and semantic-model services | Remain active because BI chat uses their errors, Dataset Trust, governed data, metrics, and lineage. |

`decision_assets` is the only dedicated durable Decision Intelligence table. It remains in the shared SQLite schema so saved compatibility snapshots are recoverable. Decision workspaces, pipeline bundles, recommendations, and scenarios are request-scoped. Decision graph state remains client-session state or an optional field inside a saved `decision_assets` snapshot.

Frontend source classification is also explicit. `frontend/frontend/src/features/ai/AIShell.jsx` is the primary caller of `/api/decision/chat/turns`. `frontend/frontend/src/features/business/decision/decisionApi.js` and `DecisionGraphWorkspace.jsx` are compatibility callers. Current source still exposes the compatibility Decision Graph through `App.jsx`, `MenuBar.jsx`, and `CanvasContainer.jsx`; that visible entrypoint must be removed or placed behind an explicitly approved compatibility mode before the default primary runtime is browser-complete.
...
The latest `user_message` is authoritative. A message reuses prior analytical state only when it supplies verified refinement evidence: a structured `analytics_refinement`, an output-only chart request, a grounded dimension value or time filter, a recognized refinement phrase, or a terse `by <dimension>` grouping. A message that names a metric starts a new analysis and clears prior grouping, filters, and output preference. A general dataset question clears incompatible analytical state and uses the grounded reply path instead of allowing the raw parser to guess a previous measure. BI session state does not retain `decision_prompt`.
