...
Output rules: Return JSON only. Do not include Markdown around the JSON. Do not include commentary before or after the JSON. Use stable identifiers for recommendations, risks, disagreements, questions, and phases. Every recommendation must be traceable to at least one participating agent and at least one reviewed source. Every implementation phase must include entry criteria, exit criteria, and validation steps. Mark Codex as Lead Orchestrator and owner of roadmap, gates, backend logic, contracts, tests, active markdown coordination, handoff scope, and integration review. If a recommendation requires frontend code, mark Antigravity as the bounded implementation owner under a Codex-authored handoff unless the user explicitly authorizes Codex frontend work for that session.
