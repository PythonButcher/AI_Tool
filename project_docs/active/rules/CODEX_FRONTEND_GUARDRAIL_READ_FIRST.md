...
- do backend support work and create a bounded Antigravity handoff instead
...
Before Codex creates any Antigravity frontend implementation handoff, Codex must verify and state the backend readiness level from source, not from assumptions:
...
Every substantial AI Tool wrap-up must state exactly whose turn is next: Codex, Antigravity, user validation, or no one. Codex determines that next owner from verified project evidence. If Antigravity is next, Codex must put the `Goal:` prompt in the active handoff file and reference that file in the final response instead of pasting the full prompt into chat.
...
The leash is intentionally bounded, not prescriptive. Codex fixes the product outcome, API and state boundary, must-pass behavior, prohibited scope, and verification evidence. Antigravity may choose component structure, accessible control treatment, spacing, visual emphasis, micro-interactions, and concise copy when those choices stay inside the existing design system and do not alter backend truth or broaden the product scope.

If Antigravity reports that the requested scope is too large or ambiguous, Codex must reduce it into ordered slices before implementation continues. Do not use extra agents to conceal an oversized frontend handoff.
...
- Inspect only the frontend files named by the handoff or Antigravity summary.
...
- Run `npm --prefix frontend\frontend run build` only when Codex finds a likely syntax/import error, Antigravity did not report a build, or the user explicitly asks for build verification.
