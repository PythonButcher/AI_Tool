...
When explaining plans, rollouts, architecture choices, or agent handoffs, Codex must write in plain project language. Do not compress important meaning into dense phrases such as "Decision Map now, CDD later" unless the phrase is immediately explained in simple terms.

For rollout plans, use short phase names and one clear purpose per phase. Say what gets built, who owns it, what proves it works, and what waits. Avoid walls of bullets, overloaded tables, and abstract wording when the user needs an implementation plan they can quickly review.

If the user says a phrase is confusing, first explain exactly what was meant in direct language, then rewrite the plan in the clearer style before continuing.
